 ...
 * == Main ==
 * 
 * 	Main Uncharted Library Section
 * 
 * 	Uncharted is intended as a cross browser charting library based on
 * 	ProtoChart and g.raphael.
 * 
 * Essentially I liked the look and ease of use of ProtoChart but wanted 
 * some of the cross browser interactivity of g.raphael so I decided to 
 * write my own library combining the aspects I liked of the two and adding
 * in and tweaking other bits to suit the way I wanted things to work.
 * 
 * The library is built on Prototype as this is what I use for all my Ajax 
 * stuff and I wanted to be able to use some of the helpers things in the 
 * library.  It also uses the Raphael SVG library for graphics.  I decided 
 * to go with SVG over Canvas as it is vector based and therefore should 
 ...
 * Uncharted
 * 
 * The default namespace for the library
 **/
var Uncharted = {};

/**
 * class Uncharted.base
 * 
 * This is the abstract class that defines the default options and cross chart methods
 * for each chart type.
 **/
Uncharted.base = Class.create({
    /**
     * new Uncharted.base(element,data[,options])
     * - element (String): The DOM element to create the canvas on.
     * - data (Object): The data to be displayed.
     * - options (Object | null): Options to override the defaults. 
     * 
     * Creates a new instance of the base class.
     * 
     * This method is inherited and called by all 
     * chart types as part of their own constructor functions
     **/
     ...
     * - destination (Object): `Object` to have properties overridden.
     * - source (Object): `Object` containing new definitions for properties.
     * 
     * Returns an extended `Object` with all properties of destination overwritten with 
     * properties from source. Extends the functionality of Prototype's built in `Object.Extend` 
     * method by enabling properties that are `Objects` to only have the corresponding properties 
     * from `source` overwritten.
     **/
    ...
    /**
     * Uncharted.base.parseData() -> Object
     * 
     * Takes the data originally passed to the constructor and converts it to a consistent 
     * format.
     * 
     * Returns an object in the format {\n
     * 									total:%total data% (only used in pie charts)\n
     * 									data: %array of objects containing data for the chart%\n
     * 								   }\n
     **/
 	parseData: function(){
    ...
    /**
     * Uncharted.base.drawGrid() -> Raphael Object
     * 
     * Draws a background grid based on the axis tick points.
     **/
 	drawGrid: function(){
    ...
    /**
     * Uncharted.base.getMarker(x,y,options,color) -> Raphael Object
     * - x (Float): Defines the x coordinate of the marker.
     * - y (Float): Defines the y coordinate of the marker.
     * - options (Object): `Object` containing size and style specifications for the marker.
     * - color (String): Defines the fill colour of the marker.
     * 
     * Draws a marker at the given coordinates and returns the `Raphael` `Object` that
     * corresponds to that marker.
     **/
 	...
 	/**
 	 * Uncharted.base.showTooltip(x,y[,text]) -> Raphael Object
 	 * - x (Float): Defines the x coordinate of the tooltip.
     * - y (Float): Defines the y coordinate of the tooltip.
     * - text (String): Text to display inside the tool tip.  Defaults to `x,y`.
     * 
     * Displays a tool tip at the given coordinates with the supplid text and returns
     * a the corresponding `Raphael` `Object`.
 	 **/
...
/**
 * class Uncharted.pie < Uncharted.base
 * 
 * Creates a pie chart
 **/
Uncharted.pie = Class.create(Uncharted.base,{
	/**
	 * new Uncharted.pie(element,data [,options])
	 * - element (String): The DOM element to create the canvas on.
	 * - data (Object): The data to be displayed.
	 * - options (Object | null): New options to override defaults.
     * 
