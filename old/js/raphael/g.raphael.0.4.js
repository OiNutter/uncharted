...
(function () {
    ...
    Raphael.fn.g.axis = function (x, y, length, from, to, steps, orientation, labels, type, dashsize,grid) {
        ...
        type = type || "t";
        steps = steps || 10;
        var path = gridLine =  type == "|" || type == " " ? ["M", x + .5, y, "l", 0, .001] : orientation == 1 || orientation == 3 ? ["M", x + .5, y, "l", 0, -length] : ["M", x, y + .5, "l", length, 0],
        ...
        if (+orientation == 1 || +orientation == 3) {
            ...
            while (Y >= y - length) {
                ...
                if(grid){
                	...
                	X = x - (type == "+" || type == "|" ? dashsize : !(orientation - 1) * dashsize * 2);
                	gridLine = gridLine.concat(["M", X, Y + .5, "l", this.width-X*2, 0]);
            ...
            while (X <= x + length) {
                ...
                if(grid)
                  	gridLine = gridLine.concat(["M", X + .5, 0 + (this.height-y), "l", 0, y - (type == "+" ? dashsize : !!orientation * dashsize * 2)]);
